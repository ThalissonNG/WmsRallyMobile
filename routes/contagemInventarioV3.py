...
def contagem_inventario_v3(page, navigate_to, header):
    ...
    def focar(control):
        try:
            ...
            print("Erro ao focar campo V3:", exc)
    ...
    def limpar_estado():
        ...
        state["mostrar_cadastro_codbarra"] = False
        busca_endereco_field.value = ""
        busca_endereco_field.error_text = None
    ...
    def carregar_resumo(silencioso=False):
        ...
        try:
            ...
            print("Erro ao atualizar resumo V3:", exc)
            if not silencioso:
    ...
    def render_separar():
        etapa_label = {
            "inicial": "Escolha um endereço específico ou carregue o próximo automaticamente.",
            ...
            controls.append(
                ft.Container(
                    ...
                    content=ft.Column(
                        spacing=10,
                        controls=[
                            ft.Text("Localizar endereço", weight="bold"),
                            busca_endereco_field,
                            ft.Row(
                                spacing=10,
                                controls=[
                                    buscar_endereco_btn,
                                    proximo_automatico_btn,
                                ],
                            ),
                        ],
                    ),
    ...
    def definir_busca_em_andamento(em_andamento):
        buscar_endereco_btn.disabled = em_andamento
        proximo_automatico_btn.disabled = em_andamento
        page.update()

    def carregar_os(dados_os, mensagem=None, cor=None):
        limpar_estado()
        ...
        if mensagem:
            snack_bar(
                mensagem,
                cor or colorVariaveis["sucesso"],
                colorVariaveis["textoPreto"] if cor != colorVariaveis["erro"] else colorVariaveis["texto"],
                page,
            )
        focar(endereco_field)

    def buscar_inventario(e, modo_manual):
        codendereco = (busca_endereco_field.value or "").strip()
        if modo_manual and (not codendereco or not codendereco.isdigit()):
            busca_endereco_field.error_text = "Informe um código de endereço válido."
            busca_endereco_field.update()
            focar(busca_endereco_field)
            return

        busca_endereco_field.error_text = None
        definir_busca_em_andamento(True)
        try:
            payload = {"codfilial": codfilial, "matricula": matricula}
            if modo_manual:
                payload["codendereco"] = codendereco
            ...
            dados = response.json()
            mensagem = dados.get("mensagem") or "Não foi possível buscar o inventário."
            ...
            if response.status_code in [200, 202]:
                ...
                if not dados_os:
                    ...
                    return
                carregar_os(dados_os, mensagem)
                return

            if response.status_code == 409 and codigo == "OS_PENDENTE" and dados_os:
                carregar_os(dados_os, mensagem, colorVariaveis["erro"])
                return

            snack_bar(mensagem, colorVariaveis["erro"], colorVariaveis["texto"], page)
        except Exception as exc:
            print("Erro ao buscar inventário V3:", exc)
            snack_bar(
                "Erro de comunicação ao buscar o inventário.",
                colorVariaveis["erro"],
                colorVariaveis["texto"],
                page,
            )
        finally:
            definir_busca_em_andamento(False)
    ...
    def validar_codbarra(e):
        ...
        try:
            ...
            print("Erro ao validar código de barras V3:", exc)
    ...
    def confirmar_quantidade(e):
        ...
        try:
            ...
            print("Erro ao confirmar quantidade V3:", exc)
    ...
    def salvar_edicao(item):
        ...
        try:
            ...
            print("Erro ao editar item V3:", exc)
            snack_bar("Erro ao atualizar quantidade", colorVariaveis["erro"], colorVariaveis["texto"], page)
    ...
    def confirmar_finalizacao(dialog, btn_confirmar):
        ...
        try:
            ...
            print("Erro ao finalizar inventário V3:", exc)
            btn_confirmar.disabled = False
    ...
    title = ft.Text("Inventário V3", size=24, weight="bold", color=colorVariaveis["titulo"])

    busca_endereco_field = ft.TextField(
        label="Código do endereço",
        hint_text="Digite ou leia o endereço",
        autofocus=True,
        keyboard_type=ft.KeyboardType.NUMBER,
        on_submit=lambda e: buscar_inventario(e, True),
    )
    buscar_endereco_btn = ft.ElevatedButton(
        "Buscar endereço",
        bgcolor=colorVariaveis["botaoAcao"],
        color=colorVariaveis["texto"],
        expand=True,
        on_click=lambda e: buscar_inventario(e, True),
    )
    proximo_automatico_btn = ft.ElevatedButton(
        "Próximo automático",
        expand=True,
        on_click=lambda e: buscar_inventario(e, False),
    )
