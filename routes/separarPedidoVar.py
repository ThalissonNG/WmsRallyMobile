...
def separar_pedido_var(page: ft.Page, navigate_to, header, arguments=None):
    print(f"Separar pedido var")
    ...
    def buscar_itens_pedido(numped, codfilial):
        ...
        response = requests.get(
            f"{base_url}/separarPedidoVar/{numped}",
            params={
                "codfilial": codfilial,
            }
        )
        ...
        if response.status_code == 200:
            ...
            print(f"Resumo do pedido {numped}: {itens}")
    ...
    def construir_endereco(item):
        ...
        nivel_item = item[12]
        apto_item = item[13]
        tipo_entrega = item[15] if item[15] is not None else "Entrega Própria"
    ...
    def contruir_produto(item):
        ...
        def validar_codbarras(e, codprod_item, codbarra):
            ...
            try:
                response = requests.get(
                    f"{base_url}/separarPedidoVar/{pedido}",
                    params={
                        "codfilial": codfilial,
    ...
    def finalizar_separacao(numped):
        try:
            ...
            if response.status_code == 200:
                mensagem = resposta.get("message")
                snack_bar(mensagem, colorVariaveis['sucesso'], colorVariaveis['textoPreto'], page)
                navigate_to("/buscar_pedido_varejo")
            else:
                mensagem = resposta.get("message")
                snack_bar(mensagem, colorVariaveis['erro'], colorVariaveis['texto'], page)
        except Exception as e:
