...
def buscar_transferencia_devolucao(e, navigate_to, header):
    ...
    def transferencia_devolucao(codfornec, numnota, codfilial):
        try:
            response = requests.post(
                f"{base_url}/consultar_transferencia_devolucao",
                json={
                    "numnota": numnota,
                    "codfornec": codfornec,
                    "codfilial": codfilial,
    ...
    title = ft.Text(
        "Buscar Transferencia/Devolução",
        size=24,
        weight="bold",
    ...
    return ft.View(
        route="/buscar_transferencia_devolucao",
        controls=[
            header,
            title,
